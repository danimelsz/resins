...
# 2.6 PHYLOGENY
tree = read.nexus(file = "data/tree_edited.nex")
...
itd.data = itd.data[c(1,2, 12, 14, 21, 23)] # select columns of species and ITD
...
# 4.1 Fitting evolutionary models
...
itd.data.d = itd.data[-c(20,39),] # remove rows with -Inf values in log d'
...
itd.data.r = itd.data[-c(6,8, 16, 18, 20, 26, 28, 35, 38, 40),]
...
itd.data.b = itd.data[-c(6, 8, 13, 18, 20, 26, 28, 35, 38, 40),] # remove rows with -Inf values in log b
...
c_setnames = setNames(itd.data$log_c,rownames(itd.data)) # create a vector with log r associated with species names
...
pglsOU.r1 = gls(log_r~1, correlation = corMartins(1, phy=tree.cut.r), data = itd.data, method="ML")
pglsOU.r2 = gls(log_r~log_ITD, correlation = corMartins(1, phy=tree.cut.r), data = itd.data, method="ML")
...
# 4.2.4 CLOSENESS
pglsOU.c1 = gls(log_c~1, correlation = corMartins(1, phy=tree.cut.c), data = itd.data, method="ML")
pglsOU.c2 = gls(log_c~log_ITD, correlation = corMartins(1, phy=tree.cut.c), data = itd.data, method="ML")
...
###################################################
# TABLE S3: LMs INCLUDING ITD AND SAMPLING EFFORT #
###################################################
...
#########################################
# TABLE S4: CORRELATION BETWEEN METRICS #
#########################################
